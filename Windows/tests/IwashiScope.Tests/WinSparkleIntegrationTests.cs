...
public sealed class WinSparkleIntegrationTests
{
    ...
    public void ReleaseBuildCreatesAnInstallableWinSparklePayload()
    {
        ...
        Assert.Contains("<sparkle:version>$currentBuildVersion</sparkle:version>", signingTestScript);
        Assert.Contains("item[0].shortVersionString -ne $currentVersion", signingTestScript);
    ...
    public void LifecycleConfiguresSecurityCallbacksAndCleanup()
    {
        ...
        Assert.Equal(typeof(WinSparkleUpdater).Assembly.GetName().Version!.ToString(), native.BuildVersion);
