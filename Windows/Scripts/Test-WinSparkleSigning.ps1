...
try {
    ...
    $historicalXml = $generated.OuterXml.Replace(
        '<sparkle:version>0.9.5.2</sparkle:version>',
        '<sparkle:version>0.9.5.1</sparkle:version>').Replace(
        '<sparkle:shortVersionString>0.9.5.2</sparkle:shortVersionString>',
        '<sparkle:shortVersionString>0.9.5.1</sparkle:shortVersionString>').Replace(
        'IwashiScope-0.9.5.2-Windows-x64-Setup.exe',
        'IwashiScope-0.9.5.1-Windows-x64-Setup.exe')
    ...
    if ($historyAppcastItemCount -ne 2 -or
        $historyGenerated.rss.channel.item[0].version -ne '0.9.5.2' -or
        $historyGenerated.rss.channel.item[1].version -ne '0.9.5.1') {
