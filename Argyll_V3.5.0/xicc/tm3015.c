...
double psamples[IES_TM_30_15_ESAMPLES][2][3],	/* If not NULL, return each ref & tsamp Jab */
...
# pragma message("###### tm3015.c set to 5nm !!! ######")
	...
	if (psamples != NULL) {
		for (i = 0; i < IES_TM_30_15_ESAMPLES; i++) {
			icmCpy3(psamples[i][0], esamps[i][0]);
			icmCpy3(psamples[i][1], esamps[i][1]);
		}
	}
