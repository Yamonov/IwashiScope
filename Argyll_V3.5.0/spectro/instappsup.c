...
/* is provided. */
inst_code inst_handle_calibrate_ex(
	inst *p,
	...
	inst_app_calibration_callback callback,
	...
	if (callback != NULL)
		callback(callback_context, p, inst_app_calibration_started,
		         calt, calc, inst_calc_id_none, NULL, inst_ok);
	...
	for (;;) {
		...
		if ((ev & inst_mask) == inst_ok) {
			...
			if (callback != NULL)
				callback(callback_context, p, inst_app_calibration_complete,
				         calt, calc, idtype, id, ev);
			return ev;
		}
		...
		if ((ev & inst_mask) == inst_user_abort) {
			...
			if (callback != NULL)
				callback(callback_context, p, inst_app_calibration_aborted,
				         calt, calc, idtype, id, ev);
			return ev;
		}
		...
		if ((ev & inst_mask) != inst_cal_setup) {
			if ((ev & inst_mask) == inst_unsupported) {
				...
				if (callback != NULL)
					callback(callback_context, p, inst_app_calibration_failed,
					         calt, calc, idtype, id, ev);
			...
			if (ch == 0x1b || ch == 0x3 || ch == 'q' || ch == 'Q') {
				...
				if (callback != NULL)
					callback(callback_context, p, inst_app_calibration_aborted,
					         calt, calc, idtype, id, inst_user_abort);
				return inst_user_abort;
			}
				...
				case inst_calc_emis_grey_ligher:
					if (dwi == NULL) {	/* No way of creating a test window */
						...
						if (p->cal_gy_count > 4) {
							...
							if (callback != NULL)
								callback(callback_context, p, inst_app_calibration_failed,
								         calt, calc, idtype, id, inst_internal_error);
			...
			if (callback != NULL)
				callback(callback_context, p, inst_app_calibration_prompt,
				         calt, calc, idtype, id, inst_cal_setup);
			...
			if (!doimmediately
			 && (calc & inst_calc_cond_mask) != inst_calc_man_ref_whitek) {
				...
				if ((calc & inst_calc_optional_flag) != 0 && (ch == 's' || ch == 'S')) {
					printf("Skipped\n");
					if (callback != NULL)
						callback(callback_context, p, inst_app_calibration_skipped,
						         calt, calc, idtype, id, inst_ok);
					goto oloop;
				}
				if (ch == 0x1b || ch == 0x3 || ch == 'q' || ch == 'Q') {
					a1logd(p->log,1,"inst_handle_calibrate user aborted 0x%x\n",inst_user_abort);
...
inst_code inst_handle_calibrate(
	...
	return inst_handle_calibrate_ex(
		p, calt, calc, disp_setup, dwi, doimmediately, NULL, NULL
	);
}
