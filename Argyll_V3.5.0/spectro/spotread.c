...
#ifndef SALONEINSTLIB
static spotread_jsonl *g_jsonl = NULL;
static int g_jsonl_calibration_trigger_armed = 0;
static int g_jsonl_measurement_armed = 0;
static int g_jsonl_measurement_started = 0;
...
static void jsonl_emit_measurement_started(void) {
	if (g_jsonl == NULL || g_jsonl_measurement_started)
		return;

	jsonl_check(spotread_jsonl_emit_state(g_jsonl, "measurementStarted"));
	g_jsonl_measurement_started = 1;
}

static void jsonl_emit_triggered_calibration_started(void) {
	if (g_jsonl == NULL || !g_jsonl_calibration_trigger_armed)
		return;

	g_jsonl_calibration_trigger_armed = 0;
	jsonl_check(spotread_jsonl_emit_calibration(
		g_jsonl,
		"started",
		inst_calc_none,
		inst_calc_id_none,
...
static void jsonl_calibration_callback(
	void *context,
	inst *p,
	inst_app_calibration_event event,
	inst_cal_type calt,
	inst_cal_cond calc,
	...
	if (event == inst_app_calibration_prompt
	 && (calc & inst_calc_cond_mask) == inst_calc_man_ref_whitek)
		g_jsonl_calibration_trigger_armed = 1;
	else
		g_jsonl_calibration_trigger_armed = 0;

	switch (event) {
		case inst_app_calibration_started: phase = "started"; break;
		case inst_app_calibration_prompt: phase = "prompt"; break;
		case inst_app_calibration_complete: phase = "completed"; break;
		case inst_app_calibration_failed:
			phase = "failed";
			reason = p->interp_error(p, error_code);
			break;
		case inst_app_calibration_aborted: phase = "aborted"; break;
		case inst_app_calibration_skipped: phase = "skipped"; break;
		default: phase = "failed"; reason = "Unknown calibration event"; break;
	}

	jsonl_check(spotread_jsonl_emit_calibration(
		(spotread_jsonl *)context,
		phase,
		calc,
		idtype,
		id,
...
# define SPOTREAD_HANDLE_CALIBRATE(p,calt,calc,disp,dwi,now,protocol) \
	inst_handle_calibrate_ex((p),(calt),(calc),(disp),(dwi),(now), \
	                         jsonl_calibration_callback,(protocol))
#else
# define SPOTREAD_HANDLE_CALIBRATE(p,calt,calc,disp,dwi,now,protocol) \
	inst_handle_calibrate((p),(calt),(calc),(disp),(dwi),(now))
#endif
...
static inst_code uicallback(void *cntx, inst_ui_purp purp) {
	...
	if (purp == inst_triggered) {
		jsonl_emit_triggered_calibration_started();
		if (g_jsonl_measurement_armed)
			jsonl_emit_measurement_started();
	}
	#endif
...
#ifndef SALONEINSTLIB
/* Preserve Argyll's normal keyboard handling while reporting the actual
 * instrument trigger before read_sample() performs the measurement. */
static inst_code jsonl_uicallback(void *cntx, inst_ui_purp purp) {
	inst_code rv;
	(void)cntx;

	rv = inst_get_uicallback()(inst_get_uicontext(), purp);
	if (purp == inst_triggered) {
		jsonl_emit_triggered_calibration_started();
		if (g_jsonl_measurement_armed)
			jsonl_emit_measurement_started();
...
#ifndef SALONEINSTLIB
	jsonl_check(spotread_jsonl_emit_instrument(
		protocol,
		inst_name(it->get_itype(it)),
		it->get_serial_no(it)
	));
#endif
	...
	if (lampdrift) {
		...
		for (pass = 0; pass < 2; pass++) {
			...
			if (it->needs_calibration(it) & inst_calt_n_dfrble_mask) {
				...
				ev = SPOTREAD_HANDLE_CALIBRATE(it, inst_calt_needed, inst_calc_none, NULL, NULL, doone, protocol);
...
remediate:;
	...
	if (doone) {	/* Set to trigger immediately */
		...
		it->set_uicallback(it, jsonl_uicallback, NULL);
	}
#endif
	...
	for (ix = 1;; ix++) {
		...
		g_jsonl_measurement_armed = 0;
		g_jsonl_measurement_started = 0;
#endif /* !SALONEINSTLIB */
		int ch = '0';		/* Character */
		...
		if (savdrd == 1) {
			...
			#ifndef SALONEINSTLIB
			jsonl_check(spotread_jsonl_emit_state(protocol, "savedReadingPrompt"));
			#endif
			do_fflush();
		...
		/* Do a normal read */
			...
			#ifndef SALONEINSTLIB
			jsonl_check(spotread_jsonl_emit_state(protocol, "ready"));
			g_jsonl_measurement_armed = 1;
			#endif
			do_fflush();
...
#ifndef SALONEINSTLIB
		g_jsonl_measurement_armed = 0;
		if ((rv & inst_mask) == inst_ok
		 || ((rv & inst_mask) == inst_user_trig && (ch & DUIH_CMND) == 0))
			jsonl_emit_measurement_started();
#endif
		...
		if ((rv & inst_mask) == inst_user_abort && (ch & DUIH_ABORT)) {
			#ifndef SALONEINSTLIB
			jsonl_check(spotread_jsonl_emit_issue(
				protocol, "userStopped", "Measurement stopped by the user",
				rv, "resumeMeasurementLoop"
			));
			#endif
		...
		/* Deal with a bad sensor position */
			...
			#ifndef SALONEINSTLIB
			jsonl_check(spotread_jsonl_emit_issue(
				protocol, "wrongConfiguration", it->interp_error(it, rv),
				rv, "acknowledgeConfiguration"
			));
			#endif
		...
		/* Deal with a misread */
			...
			#ifndef SALONEINSTLIB
			jsonl_check(spotread_jsonl_emit_issue(
				protocol, "misread", it->interp_error(it, rv),
				rv, "resumeMeasurementLoop"
			));
			#endif
			empty_con_chars();
		...
		/* Deal with a communications error */
			...
			#ifndef SALONEINSTLIB
			jsonl_check(spotread_jsonl_emit_issue(
				protocol, "communicationFailure", it->interp_error(it, rv),
				rv, "resumeMeasurementLoop"
			));
			#endif
			empty_con_chars();
		...
		/* Some fatal error */
			...
			#ifndef SALONEINSTLIB
			jsonl_check(spotread_jsonl_emit_issue(
				protocol, "fatalInstrumentError", it->interp_error(it, rv),
				rv, "restart"
			));
			#endif
			printf("\n\nGot fatal error '%s' (%s)\n",
		...
		if (ch == 'K' || ch == 'k') {	/* Do a calibration */
			...
			ev = SPOTREAD_HANDLE_CALIBRATE(it, inst_calt_available, inst_calc_none, NULL, NULL, doone, protocol);
			if (ev != inst_ok) {	/* Abort or fatal error */
				printf("\nSpot read got abort or error from calibrate\n");
				break;
			}
		...
		if (ch == 'f') {
			...
			if (ev == inst_unsupported) {
				...
				ev = SPOTREAD_HANDLE_CALIBRATE(it, inst_calt_ref_freq, inst_calc_none, NULL, NULL, doone, protocol);
		...
		if (sufwa == 0) {		/* Not setting up fwa, so show reading */
			...
			if (sp.spec_n > 0 && doCCT) {
				...
				protocol_measurement.has_cri = 1;
				protocol_measurement.cri_ra = cri;
				protocol_measurement.cri_caution = invalid != 0;
				for (i = 0; i < 14; i++)
					protocol_measurement.cri_individual[i] = RR[i];
			...
			if (sp.spec_n > 0 && doCCT) {
				...
				protocol_measurement.has_tlci = 1;
				protocol_measurement.tlci_qa = tlci;
				protocol_measurement.tlci_caution = invalid != 0;
			...
			if (sp.spec_n > 0 && doCCT) {
				...
				invalid = icx_IES_TM_30_15(
					&Rf,
					&Rg,
					&cct,
					&dc,
					protocol_measurement.tm30_bins,
					protocol_measurement.tm30_samples,
					&sp
				);
				protocol_measurement.tm30_status = invalid;
				if (invalid < 2) {
					protocol_measurement.tm30_rf = Rf;
					protocol_measurement.tm30_rg = Rg;
					protocol_measurement.tm30_cct = cct;
					protocol_measurement.tm30_duv = dc;
					printf(" IES TM-30-15 Rf = %.2f Rg = %.2f CCT = %.0f Duv = %f%s\n", Rf, Rg, cct, dc, invalid ? " (Caution)" : "");
				} else {
					printf(" IES TM-30-15 calculation failed\n");
				}

#ifdef DO_TM3015_PLOT
				if (invalid < 2)
					tm3015_plot(protocol_measurement.tm30_bins);
#endif
			}
			...
			if (ambient && (cap2 & inst2_ambient_mono)) {
				...
				protocol_measurement.has_xyz = 1;
				protocol_measurement.xyz[0] = XYZ[0];
				protocol_measurement.xyz[1] = XYZ[1];
				protocol_measurement.xyz[2] = XYZ[2];
				protocol_measurement.has_lab = 1;
				protocol_measurement.lab[0] = Lab[0];
				protocol_measurement.lab[1] = Lab[1];
				protocol_measurement.lab[2] = Lab[2];
				protocol_measurement.lab_white_point = labwpname;
			}
			if (ambient) {
				protocol_measurement.has_lux = 1;
				protocol_measurement.lux = XYZ[1];
				if (ambient != 2 && XYZ[1] > 0.0) {
					protocol_measurement.has_ev100 = 1;
					protocol_measurement.ev100 = log(XYZ[1]/2.5)/log(2.0);
				}
			}
			if (doCCT) {
				protocol_measurement.bad_cct = bad_cct;
				protocol_measurement.bad_planckian = bad_vct;
				protocol_measurement.bad_daylight = bad_vdt;
				if (!bad_cct) {
					protocol_measurement.has_cct = 1;
					protocol_measurement.cct = cct;
					protocol_measurement.duv = cct_sn * cct_de;
				}
				if (!bad_vct) {
					protocol_measurement.has_planckian = 1;
					protocol_measurement.planckian_kelvin = vct;
					protocol_measurement.planckian_delta_e = vct_sn * vct_de;
				}
				if (!bad_vdt) {
					protocol_measurement.has_daylight = 1;
					protocol_measurement.daylight_kelvin = vdt;
					protocol_measurement.daylight_delta_e = vdt_sn * vdt_de;
				}
			}
			jsonl_check(spotread_jsonl_emit_measurement(protocol, &protocol_measurement));
#endif	/* !SALONEINSTLIB */
