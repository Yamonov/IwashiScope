...
各ビルドの前に、macOSのSparkleとWindowsのWinSparkleについて、公式リリースとパッケージ配布元で最新の安定版を確認してください。古い固定値のまま「最新」と扱わず、必要な場合は固定バージョン、ロックファイル、通知・テストを更新し、完成したアプリに同梱されたフレームワーク／DLLのバージョンも検証します。再現性のため、最新版確認後も依存関係は厳密なバージョンに固定します。

新しい依存関係が現在の対応OSなどに非互換な変更を要求する場合は、対応条件を変更する前に確認してください。

アプリをビルドするたびにビルド番号を増やします。macOSは`CURRENT_PROJECT_VERSION`、Windowsは`Windows/Directory.Build.props`の`IwashiScopeBuildNumber`を使用し、失敗後にビルドし直す場合も増やします。
